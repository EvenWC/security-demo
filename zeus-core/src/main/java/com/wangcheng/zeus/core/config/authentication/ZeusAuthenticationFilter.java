...
public class ZeusAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(StringUtils.isEmpty(token)){
            HttpResponseUtils.printJson(response,ResponseModel.ACCESS_FORBIDDEN("未登录"));
            return;
        }
        Claims claims;
        try {
    ...
    private boolean matchExcludeURIs(HttpServletRequest request){
        String requestURI = request.getRequestURI();
        String[] excludeURIs = ZeusSecurityConstants.URI_EXCLUDE(zeusProperties.getBrowser().getLoginPage());
